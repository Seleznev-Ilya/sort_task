...
//                                  arrProduct[item]['a.ratingRevievs'] Первое решение***
function review(arr) {
    arr.sort((a, b) => {
        a.ratingRevievs = a.ratingRevievs.slice(0, a.ratingRevievs.indexOf('отзыв'));
        b.ratingRevievs = b.ratingRevievs.slice(0, b.ratingRevievs.indexOf('отзыв'));
    });
    arr.sort((a, b) => {
        return +a.ratingRevievs < +b.ratingRevievs ? 1 : -1;
    });
    arr.sort((a, b) => {
        a.ratingRevievs = a.ratingRevievs + ' отзывов';
        if (b.ratingRevievs.indexOf(' отзывов') !== -1) {
            b.ratingRevievs = b.ratingRevievs.slice(0, b.ratingRevievs.indexOf('отзыв'));
        }
        b.ratingRevievs = b.ratingRevievs + ' отзывов';
    });
    console.log(arrProduct);
}

//                                  arrProduct[item]['a.ratingRevievs'] Второе решение***
function priceCompare(arr) {
    ...
    arr.sort((a, b) => {
        if (typeof a.price === 'string') {
            one = a.price.slice(0, length - 3);
            one1 = one.replace(' ', '');
        } else if (typeof a.price === 'object') {
            one = a.price.newUan.slice(0, length - 3);
            one1 = one.replace(' ', '');
        }
        if (typeof b.price === 'string') {
            two = b.price.slice(0, length - 3);
            two1 = two.replace(' ', '');
        } else if (typeof b.price === 'object') {
            two = b.price.newUan.slice(0, length - 3);
            two1 = two.replace(' ', '');
        }
        // console.log(one1);
        a = +one1;
...
function ask() {
    let age = prompt("Сортировка:\nцена\\отзывы", 'отзывы');
    age = age.trim();
    (age === 'отзывы') ?
        review(arrProduct) : (age === 'цена') ?
        priceCompare(arrProduct) : alert("Не верно!");
}
